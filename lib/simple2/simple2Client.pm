...
sub new
{
    ...
    my $self = {
	client => simple2::simple2Client::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a simple2.SimpleParams
$output is a simple2.SimpleResults
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method simple_add",
            status_line => $self->{client}->status_line,
            method_name => 'simple_add',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for simple2::simple2Client\n";
    }
    if ($sMajor == 0) {
        warn "simple2::simple2Client version is $svr_version. API subject to change.\n";
    }
}
...
package simple2::simple2Client::RpcClient;
